...
object OdpsDataFrame extends Logging {
  ...
  def readOdpsTableForR(sparkSession: SparkSession,
                        projectName: String,
                        tabelName: String,
                        partitions: Array[String],
                        numPartition: Int = 0): RDD[Row] = {
    readOdpsTable(sparkSession.sqlContext, projectName, tabelName, partitions, numPartition)
  }

  //type is double,because R does not have Integer type
  def createOdpsDataFrameForR(sparkSession: SparkSession,
                              projectName: String,
                              tableName: String,
                              partitions: Array[String] = Array[String](),
                              numPartition: Double = 0): DataFrame = {
    createOdpsDataFrame(sparkSession.sqlContext,
      projectName,
      tableName,
      partitions,
      numPartition.toInt)
  ...
  /**
    ...
    * @param numPartition partition num for RDD to execute,0 by default
  ...
  def createOdpsDataFrame(sqlContext: SQLContext,
                          ...
                          tableName: String,
                          partitions: Array[String] = Array[String](),
                          numPartition: Int = 0): DataFrame = {
    val odpsRdd: RDD[Row] = readOdpsTable(sqlContext, projectName, tableName, partitions, numPartition)
    val odpsdf: DataFrame = OdpsRDD2DF(sqlContext, odpsRdd)
    odpsdf
  }
  ...
  private def readOdpsTable(sqlContext: SQLContext,
    ...
    val odpsRdd: RDD[Row] = odpsOps.readTable(projectName, tabelName, partitions,
  ...
  private def OdpsRDD2DF(sqlContext: SQLContext, odpsRdd: RDD[Row]): DataFrame = {
    if (odpsRdd.isInstanceOf[OdpsRDD[Row]]) {
      val realodpsRdd = odpsRdd.asInstanceOf[OdpsRDD[Row]]
      //val sqlContext = createSQLContext(jsc)
      val projectName = realodpsRdd.project
      val tableName = realodpsRdd.table
      val schema = StructType(CupidSession.get.odps.tables().get(projectName, tableName).
        getSchema().getColumns().asScala.map(
        (col: com.aliyun.odps.Column) => {
          val colname: String = col.getName()
          val coltype: OdpsType = col.getType()
          coltype match {
      ...
      sqlContext.createDataFrame(realodpsRdd, schema)
    } else {
      throw new SparkException(s"Not InstanceOf OdpsRDD[Row]] failed!")
    }
  }

  def saveODPSForR(sparkSession: SparkSession,
                   df: DataFrame,
                   projectName: String,
                   tableName: String,
                   partitionStr: String = "",
                   isOverWrite: Boolean = false): Unit = {
    saveODPS(sparkSession.sqlContext, df, projectName, tableName, partitionStr, isOverWrite)
  }

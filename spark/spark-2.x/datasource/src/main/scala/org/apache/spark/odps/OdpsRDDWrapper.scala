...
class OdpsRDDWrapper[T: ClassTag](self: RDD[T]) extends Serializable with Logging {
  def save(
            tableName: String,
            projectName: String,
            transferUser: Any,
    ...
    val tableOutputHandle = System.getenv("META_LOOKUP_NAME") match {
      case null =>
        val tmp = self.context.parallelize(1 to 1, 1).map(i => {
          TableImplUtils.getOrCreateOutputFormat.writeTable(
            projectName,
            tableName
          )
        }).collect()
    ...
    try {
      ...
      System.getenv("META_LOOKUP_NAME") match {
        case null =>
          self.context.parallelize(1 to 1, 1).map(i => {
...
object OdpsRDDWrapper {
  def fromRDD[T: ClassTag](rdd: RDD[T]): OdpsRDDWrapper[T] = {
    new OdpsRDDWrapper[T](rdd)
  }
}
